...
class MyHashTable<K,V> {
    ...
    public  V  put(K key, V value) {
      ...
      double loadFactor = this.entryCount/this.numBuckets;
      ...
      HashLinkedList<K,V> list =  (HashLinkedList<K,V>) buckets.get(hashKey);
      
      HashLinkedList<K,V> list2 = new HashLinkedList<K,V>();
      
      if (  buckets.get(hashKey).size() !=0 ) {
          ...
          list2.add(key, value);
           ...
           buckets.get(hashKey).add(key, value);
            entryCount++;
            return (V) list;
      } else {         
              buckets.get(hashKey).add(key, value);
    ...
    public V get(K key) {
        ...
        //  ADD YOUR CODE BELOW HERE
        int hashKey = hashFunction(key); 
        HashLinkedList list2 =  (HashLinkedList) buckets.get(hashKey);
        ...
        if (containsKey(key)) {
          ...
          return (V) list.getListNode(key).getValue();
    ...
    return (V) (list.remove(key)).getValue();
    ...
    {
        //   ADD YOUR CODE BELOW HERE
      HashIterator iterList = this.iterator();

     
     int numEntry = this.entryCount;
    int numBuck = this.numBuckets;
      this.clear();
      
      for ( int index = 0; index < numBuck ; index++) {
        this.buckets.add(new HashLinkedList<K,V>());
        
      }
      this.numBuckets = this.numBuckets*2;
       while(iterList.hasNext()) {
         HashNode<K,V> current = iterList.next();
         ...
         allEntries = new HashLinkedList<K,V>();
          ...
          for (int i =0; i < numBuckets; i++) {
            ...
            if (buckets.get(i).size() !=0){
              ...
              HashNode<K,V> currentNode = (buckets.get(i)).getHead();
  
              while (currentNode !=null ) {
                allEntries.add(currentNode.getKey(), currentNode.getValue());
